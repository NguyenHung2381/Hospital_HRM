...
async function logout(req, res) {
	...
	if (token) {
		await revokeToken(token).catch((err) => console.error('[logout]', err.message));
		// Chỉ ghi tên tài khoản vào nhật ký khi chữ ký hợp lệ (token hết hạn vẫn nhận)
		let payload = null;
		try {
			payload = jwt.verify(token, JWT_SECRET, { algorithms: ['HS256'], ignoreExpiration: true });
		} catch {
			// token giả/hỏng → không ghi tên
		}
		if (!payload?.purpose && Number.isInteger(payload?.id_user)) {
